...
class BlockAuto extends Block {
  ...
  optimize() {
    super.optimize();
    var dir = window.getComputedStyle(this.domNode).direction;
    if (dir == 'rtl') {
...
class SingleLineBlot extends BlockAuto {
  replace(target) {
...
class Paragraph extends BlockAuto { }
...
class FigureBlot extends BlockEmbed {
  ...
  constructor(domNode, value) {
    ...
    this.domCaption.setAttribute('dir', 'auto');
    ...
    $(this.domCaption).on('mousedown touchstart', (e) => {
      this.focusCaptionInput(e);
    });
    $(document).on('selectionchange', (e) => {
      var selection = window.getSelection();
      if (selection &&
          selection.focusNode === this.domCaption &&
  ...
  focusCaptionInput(e) {
    if (this.domCaption.classList.contains('empty')) {
      var input = this.domCaption.querySelector('.editable_input');
      if (input) {
        e.preventDefault();
        input.focus();
      }
  ...
  appendVideoNode(src) {
    ...
    //video.setAttribute('src',      'https://telegra.ph' + this.sanitize(src));
  ...
  appendIframeNode(src) {
    ...
    //iframe.setAttribute('src',               "https://telegra.ph" + this.sanitize(src).replace("https://telegra.ph", ""));
    ...
    uploadFile(file_data, (loaded, total) => {
      ...
      if (data) {
        ...
        if (file_data.type.substr(0, 6) == 'video/') {
          ...
          //video.setAttribute("src", 'https://telegra.ph' + src);
          video.setAttribute("src", src);
        } else {
          let image = this.domWrapper.querySelector('img');
          //image.setAttribute("src", 'https://telegra.ph' + src);
...
function initQuill() {
  ...
  quill.addContainer($tl_link_tooltip);
  quill.addContainer($tl_tooltip);
  quill.addContainer($tl_blocks);
  ...
  quill.on(Quill.events.EDITOR_CHANGE, function(eventType, range) {
    ...
    $tl_article.classList.toggle('title_focused', !!(formats['blockTitle'] || formats['blockAuthor']));
    checkOncePlaceholder();
  });
...
function savePage() {
  ...
  let content = domToNode($tl_editor).children;
  content[0].children.shift();
  content[0].children.shift();
  ...
  $.ajax(T.apiUrl + "/api/v0/save/", {
    ...
    success: function(data) {
      if (data.content) {
        while ($tl_editor.firstChild) {
          $tl_editor.removeChild($tl_editor.firstChild);
        }
        $tl_editor.appendChild(nodeToDom({children: data.content}));
      };
...
function checkAuth() {
  ...
  $.ajax(T.apiUrl + '/api/v0/check/', {
    ...
    success: function(data) {
      ...
      if (data.report_hash) {
        T.reportHash = data.report_hash;
        $tl_page_footer.classList.add('tl_page_footer_visible');
      }
      ...
      if (!T.pageId) {
        $tl_article.classList.add('tl_article_edit');
      ...
      if (data.auth_alert && data.short_name) {
        var alert_text = '<i class="tl_alert_success"></i><h4>Success!</h4><p>You are now logged in as <b>' + htsc(data.short_name) + '</b> in this browser.</p>';
        if (data.migrate_count > 0 && data.migrate_hash) {
          alert_text += '<p>We can also add ' + data.migrate_count + ' Telegraph page' + (data.migrate_count > 1 ? 's' : '') + ' from this browser to your account.</p>'
  ...
  $.ajax(T.apiUrl + '/migrate', {
    ...
    success: function(data) {
      if (data.migrated_count > 0) {
        showAlert('<i class="tl_alert_success"></i><h4>Success!</h4><p>Added <b>' + data.migrated_count + '</b> Telegraph page' + (data.migrated_count > 1 ? 's' : '') + ' to your account.</p><p>To see a list of your pages, talk to the <a href="https://t.me/telegraph" target="_blank">@Telegraph</a> bot on Telegram.</p>', {
          close_btn: 'Continue'
        });
      } else {
        hideAlert();
...
function isEdit() {
  return $tl_article.classList.contains('tl_article_edit');
}
...
function updateEditable(is_editable) {
  $tl_article.classList.toggle('tl_article_edit', is_editable);
  updateEditableText();
  if (window.quill) {
  ...
  if (!is_editable) {
    var $tl_title  = $('h1', $tl_content);
    var $tl_author = $('address', $tl_content);
    var author_url = $('address a', $tl_content).attr('href');
    $('h1', $tl_header).text($tl_title.text());
    $('address a', $tl_header).text($tl_author.text());
    if (author_url) {
...
function updateReportFormState() {
  let form = $report_form.get(0);
  let disabled = (!form.reason.value || form.reason.value == 'copyright');
  $report_form.classList.toggle('copyright_selected', form.reason.value == 'copyright');
  $report_form.classList.toggle('form_disabled', !!disabled);
...
function sendReport() {
  if ($report_form.classList.contains('form_disabled')) {
    return false;
  }
  let form = $report_form.get(0);
  $report_popup.classList.add('tl_popup_loading');
  $.ajax(T.apiUrl + '/report', {
    ...
    success: function(data) {
      $report_popup.classList.remove('tl_popup_loading');
      hidePopup();
...
let $tl_page_footer   = document.querySelector('.tl_page_footer');
...
let $tl_tooltip       = document.querySelector('#_tl_tooltip');
let $tl_blocks        = document.querySelector('#_tl_blocks');
let $tl_link_tooltip  = document.querySelector('#_tl_link_tooltip');

let $bold_button      = document.querySelector('#_bold_button');
let $italic_button    = document.querySelector('#_italic_button');
let $link_button      = document.querySelector('#_link_button');
let $header_button    = document.querySelector('#_header_button');
let $subheader_button = document.querySelector('#_subheader_button');
let $quote_button     = document.querySelector('#_quote_button');
...
let $report_button    = document.querySelector('#_report_button');
let $report_popup     = document.querySelector('#_report_popup');
let $report_form      = document.querySelector('#_report_form');
let $report_cancel    = document.querySelector('#_report_cancel');
...
$tl_tooltip.addEventListener('mouseover', function(e) {
  let button = e.target;
  if (e.target.tagName == 'BUTTON' &&
      !e.target.classList.contains('disabled')) {
    $tl_tooltip.setAttribute('data-hover', button.id);
    $tl_tooltip.classList.add('hover');
    setTimeout(() => { $tl_tooltip.classList.add('hover_anim'); }, 1);
    clearTimeout($tl_tooltip.to);
  }
});

$tl_tooltip.addEventListener('mouseout', function(e) {
  let button = e.target;
...
$bold_button.addEventListener('click', function(e) {
  let input = e.target;
...
$italic_button.addEventListener('click', function(e) {
  let input = e.target;
  let active = input.classList.contains('active');
  e.preventDefault();
  let range = quill.getSelection(true);
...
$subheader_button.addEventListener('click', function(e) {
  let input = e.target;
  let active = input.classList.contains('active');
  e.preventDefault();
  let range = quill.getSelection(true);
...
$report_cancel.addEventListener('click', function() {
  hidePopup();
});*/
...
function showLinkTooltip(link, value) {
  ...
  $tl_link_tooltip.textContent = decodeURI(value);
...
function hideLinkTooltip() {
  $tl_link_tooltip.classList.remove('move_anim');
  $tl_link_tooltip.classList.remove('shown');
}
...
function hideFormatTooltip() {
  $tl_tooltip.classList.remove('move_anim');
  $tl_tooltip.classList.remove('shown');
  tooltipUpdatePosition($tl_link_tooltip, null, linkTTOptions);
}
...
function showAlert(html, options) {
  ...
  hideAlert();
  var $alert = $('<div class="tl_popup tl_alert"><main class="tl_popup_body tl_alert_message"><section></section><aside class="tl_popup_buttons"></aside></main></div>');
  ...
  if (options.submit_btn) {
    ...
    $submit_btn.html(options.submit_btn).click(function() {
      $alert.classList.add('tl_popup_loading');
      options.submit();
    }).appendTo($aside);
  }
  $alert.appendTo('body');
...
function hidePopup() {
  $('.tl_popup').classList.add('tl_popup_hidden');
}
function showPopup($popup) {
  hideAlert();
  $popup.classList.remove('tl_popup_hidden');
}
...
function tooltipUpdatePosition($tooltip, range, options) {
  ...
  $tooltip._top = tt.top;
  $tooltip.style.left = tt.left,
  $tooltip.style.top = tt.top,
...
new Image().src = window.devicePixelRatio >= 2 ? T.apiUrl + "/front/assets/images/icons_2x.png" : T.apiUrl + "/front/assets/images/icons.png";
